...
func (f *TMainForm) OnFormCreate(sender vcl.IObject) {
	...
	f.Font().SetSize(12)
	//fmt.Printf("%d %d\n %d %d %d %d %d %d\n%d %d\n", leftLabel, leftInput,
	//	leftok, M1, M2, M3, M4, M5, leftAbout, leftLtoday)
	//
...
func (f *TMainForm) OnBtnAboutClick(_ vcl.IObject) {
	vcl.ShowMessage("https://github.com/Aquarian-Age/ccal/tree/amrta/govcl")
}
...
func (f *TMainForm) show() {
	...
	var y, m, h int
	if len(text) != 10 {
		vcl.ShowMessage("年月日时数字错误 eg: 2020010203")
		os.Exit(0)
	}
	years := text[:4]
	months := text[4:6]
	days := text[6:8]
	hours := text[8:10]
	fmt.Printf("years:%s months:%s days:%s hours:%s\n", years, months, days, hours)
	y, err := strconv.Atoi(years)
	if err != nil {
		fmt.Println("years", err)
	}
